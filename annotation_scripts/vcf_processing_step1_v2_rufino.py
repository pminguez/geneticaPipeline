...
header_newvalues = ['Filter','QUAL','DP','AC','AF','AN','ExcessHet','FS','MLEAC','MLEAF','MQ','QD','BaseQRankSum','ClippingRankSum','MQRankSum','ReadPosRankSum','SOR','GT','AD','GQ','PL']
...
for i in range(0,len(vcfFile)):
	...
	field2split = vcfFile_lines[171]
	field2split_2 = vcfFile_lines[173]
	#field2split = vcfFile_lines[97]
	#field2split_2 = vcfFile_lines[99]
	field2split_2 = field2split_2.split(':')
	QUAL = vcfFile_lines[169]
	DP = vcfFile_lines[163]
	Filter = vcfFile_lines[170]
	...
	if len(field2split_2) < 5:
		...
		AD = field2split_2[1][field2split_2[1].rfind(',')+1:]
		GQ = field2split_2[3]
	...
	vcfFile_lines = list(vcfFile_lines[0:161])
	
	
	for element in [Filter,QUAL,DP,AC,AF,AN,ExcessHet,FS,MLEAC,MLEAF,MQ,QD,BaseQRankSum,ClippingRankSum,MQRankSum,ReadPosRankSum,SOR,GT,AD,GQ,PL]:
		if element == '':
			element = '.'
