...
for i in range(0,len(forward_paths)):
	...
	call('bwa mem -t8 -R "@RG\tID:sample\tLB:library\tPL:illumina\tPU:library\tSM:sample" ' + genome_ref + ' ' + forward_paths[i] + ' ' + reverse_paths[i] + ' | \
		samtools view -Shu - | \
...
variants = sorted(glob(sys.argv[1] + '*_singleGT_raw.vcf'))
for vcffile in variants:
	sample_path = vcffile[:vcffile.rfind('/')+1]
	sample_name = vcffile[vcffile.rfind('/')+1:vcffile.rfind('_singleGT_raw.vcf')]
	...
	call('python /mnt/genetica/domin/GeneticaPipeDB/pipeline/annotation_scripts/vcf_processing_step1_cancer.py ' + sample_path + '/' + sample_name + '.hg19_multianno.txt' ,shell = True)
