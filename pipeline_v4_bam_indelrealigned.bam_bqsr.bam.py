...
parser.add_argument("-J", action="store",dest='parallelization', type = int, default = 3,
...
# #Importing samples
# forward_paths = sorted(glob(args.input + '*R1.fastq.gz'))
# reverse_paths = sorted(glob(args.input + '*R2.fastq.gz'))
...
## Edit for each case
sample_path = "/mnt/datos2/NGS_data/RamonAreces2017_TSO/03-0274_bwa.sam_sorted.bam_dedupped.bam_indelrealigned.bam_bqsr.bam"

call("find " + sample_path + " | parallel -j" + str(args.parallelization) + " 'java -Xmx9g -jar " + gatk + " \
	-T HaplotypeCaller \
	-R " + genome_ref + " \
	-I {} \
